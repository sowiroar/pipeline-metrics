function [candidateSignals, signalType, signalNumbers, ...
                    signalLabels, params]  = getCandidateICs_vJM(EEG, eyeDetector, params)
% Extract candidate IC activations from EEG structure and filter
%
% Parameters:
%     EEG               an EEGLAB EEG structure (with ICA computed)
%     eyeDetector       Object created by eyeCatch (as parameter to improve speed)
...
EEG1.icaact = (EEG1.icaweights*EEG1.icasphere)*EEG1.data(EEG1.icachansind, :);  %JM: Comments to avoid spatial errors

%% Initialize eyeCatch - JM: Passed as parameter, no need to
...
ICindices = 1:size(EEG1.icaact,1);
