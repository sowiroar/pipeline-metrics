function [status, EEG] = f_step6InterpolateBadChans(pathStep5, nameStep5, pathStep1, nameStep1)
%Description:
%Function that performs interpolation of bad channels
%INPUTS:
%pathStep5 = Path of the .set without noisy components
%nameStep5 = Name of the .set without noisy components
...
for i = 1:length(badChanInfo.badChanIdxs)
    ...
    if ~strcmp(chanLbls{badChanInfo.badChanIdxs(i)}, badChanInfo.badChanLbls{i})
        ...
        disp('Do not correspond to the channel labels of the .set without noisy components (Step 5) at the following path:');
        disp(fullfile(pathStep5, nameStep5));
...
if ~isempty(badChanInfo.badChanIdxs)
    ...
    for i = 1:length(finalBadChanLbls)
        ...
        if ~ismember(newLbls{finalBadIdxs(i)}, badChanInfo.badChanLbls)
            ...
            disp('To try to solve this, check the function f_step5InterpolateBadChans.m');
            status = 0;
            return
        end
    end
