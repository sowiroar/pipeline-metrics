function [normalizedData, outDivisor] = f_NormalizeDataExtended(data, metric)
%Description:
...
%OUTPUTS:
%normalizedData = Matrix of [Channels, M, N] (same dimensions as the input) after normalization 
%outDivisor = The number of vector that is used to normalize (the denominator of) the data
%Author: Jhony Mejia
...
if strcmp(metric, 'UN_ALL')
    ...
    normalizedData = data./robustStd;
    outDivisor = robustStd;
...
elseif strcmp(metric, 'PER_CH')
    ...
    normalizedData = data./madPerChannels';
    outDivisor = madPerChannels';
...
elseif strcmp(metric, 'UN_CH_HB')
    ...
    %Normalizes the data
    normalizedData = data./huberMean;
    outDivisor = huberMean;
...
elseif startsWith(metric, 'RSTD_EP')
    ...
    if strcmp(metric, 'RSTD_EP_Mean')
        ...
        %Normalizes per epoch/trial
        normalizedData = permute(data, [3,2,1]);        %Moves to [subject, time, channels]
        normalizedData = normalizedData./iMean';        %Normalizes with mean per subject
        normalizedData = permute(normalizedData, [3,2,1]);        %Returns to [channels, time, subject]
    ...
    elseif strcmp(metric, 'RSTD_EP_Huber')
        ...
        %Normalizes per subject
        normalizedData = permute(data, [3,2,1]);        %Moves to [epoch, time, channels]
        normalizedData = normalizedData./iMean';        %Normalizes with mean per epoch
        normalizedData = permute(normalizedData, [3,2,1]);        %Returns to [channels, time, epoch]
    ...
    elseif strcmp(metric, 'RSTD_EP_L2')
        ...
        %Normalizes per epoch/trial
        normalizedData = permute(data, [3,2,1]);        %Moves to [subject, time, channels]
        normalizedData = normalizedData./iNorm';        %Normalizes with mean per subject
        normalizedData = permute(normalizedData, [3,2,1]);        %Returns to [channels, time, subject]
